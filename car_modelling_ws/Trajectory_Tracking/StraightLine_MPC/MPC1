%% StraightLine Trajectory Tracking using MPC 
clc;
clear;
close all;
yalmip('clear')
...
params.ninputs = 1; % No of Inputs in the system [delta_f v], steering angle
...
params.Q = [1 0 0;
            0 1 0;
            0 0 1];   % The Q matrix for running cost for error in trajectory tracking
params.Q_N = [1 0 0;
              0 1 0;
              0 0 1]; % The Q matrix for terminal cost for error in trajectory tracking
% params.R = [1 0;
%             0 1]; %  The R matrix for running cost for input
params.R = 1;
...
u(1,:) = [0,0]; % Control Input

% Generate the trajectory to be followed
traj =  [linspace(1,11,360)'  linspace(0,0,360)' linspace(0,0,360)'];
% Obtain the symbolic form of the matrices A, B
syms xkn ykn psikn xk yk psik Ts lr lf delta v
...
while (k < params.N)
  ...
  for i = 1:params.Np+1
    if i==1
        refs(i,:) = [z_ref(k,1)+0.025, 0, 0];
    else
        refs(i,:) = [refs(i-1,1)+0.025, 0, 0];
    end
    
  end
% The first case
if (k == 1)
  params.A = subs(symbolic_linear_A, [Ts lr lf psik delta v], [params.Ts params.l_r params.l_f z(k,3) 0 0]);
  params.B = subs(symbolic_linear_B, [Ts lr lf psik delta v], [params.Ts params.l_r params.l_f z(k,3) 0 0]);
...
if (k > 1)
  %Completing a circle? 
%   Increase the orientation by 2*pi so that it is always increasing along the trajectory.
  if (z_ref(k,3) < z_ref(k-1,3))    
    refs(:,3) = refs(:,3) + 2*pi;
    z_ref(k,3) = z_ref(k,3) + 2*pi;
    traj(idx,3) = traj(idx,3) + 2*pi;
  end
  params.A = subs(symbolic_linear_A, [Ts lr lf psik delta v], [params.Ts params.l_r params.l_f z(k,3) u(k-1,1) u(k-1,2)]);
  params.B = subs(symbolic_linear_B, [Ts lr lf psik delta v], [params.Ts params.l_r params.l_f z(k,3) u(k-1,1) u(k-1,2)]);
  params.A = double(params.A);
  params.B = double(params.B);
end
 ...
 yalmip('clear')
  ...
  for i = 1:params.Np
    ...
    if i < params.Np
      ...
      % Mode_mpc(params.Np+1,l constraints
      constraints = [constraints, ...
        z_mpc(i+1,:)' == params.A * z_mpc(i,:)' + params.B * u_mpc(i,:)'];

      % Input constraints
      constraints = [constraints, ...
        -pi/3 <= u_mpc(i,1) <= pi/3];% -1000 <= u_mpc(i,2) <= 1000];
    else
      J = J + ...
  ...
  z(k+1,:) = vehicle_dynamic_model(z(k,:), u(k,:), params);
...
%% Vehicle Model
function z_new = vehicle_dynamic_model(z_curr, u_curr, params)
%% Vehicle Dynamic Model
    
  delta = u_curr(1);
  vel = params.v0;
  % vehicle dynamics equations
  z_new(1) = z_curr(1) + (vel*cos(z_curr(3)+atan((params.l_r/(params.l_r+params.l_f))*tan(delta))))*params.Ts;
  z_new(2) = z_curr(2) + (vel*sin(z_curr(3)+atan((params.l_r/(params.l_r+params.l_f))*tan(delta))))*params.Ts;
  z_new(3) = z_curr(3) + (vel / params.l_r * sin(atan((params.l_r/(params.l_r+params.l_f))*tan(delta))))*params.Ts;
